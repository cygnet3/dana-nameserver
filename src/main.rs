...
async fn fetch_sp_address_from_txt_record(
    ...
    let core_network = match network {
        ...
        SpNetwork::Regtest => {
            return Err(Bip353Error::InvalidAddress(
                "Don't allow for regtest address".to_string(),
            ));
        }
    };
    ...
    let socket_addr = dns_config.dns_resolver;
    ...
    match payment_instructions {
        PaymentInstructions::ConfigurableAmount(instructions) => {
            ...
            let fixed_amt_instructions =
                instructions.set_amount(dummy_amount, &hrn_resolver).await?;
    ...
    let network_key = match sp_address.get_network() {
        ...
        SpNetwork::Regtest => {
            return (
                ...
                AxumJson(RegisterResponse {
                    id: request.id,
                    message: "Can't register regtest addresses".into(),
                    dana_address: None,
                    sp_address: None,
                    dns_record_id: None,
                }),
    ...
    let user_name = match request.user_name {
        ...
        None => {
            return (
                ...
                AxumJson(RegisterResponse {
                    id: request.id,
                    message: "User name is required".into(),
                    dana_address: None,
                    sp_address: None,
                    dns_record_id: None,
                }),
...
async fn main() {
    ...
    match list_bitcoin_records(&zone_id, &api_token).await {
        Ok(records) => {
            ...
            info!(
                "Map population complete: {} entries created, {} records processed, {} skipped, {} errors",
                map.len(),
                processed_count,
                skipped_count,
                error_count
